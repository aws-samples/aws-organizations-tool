...
def commit(
    ...
    input_folder: Folder which contains the files to commit. The whole content of input_folder will be committed
    ...
    branchName: Name of the branch to which it should be committed
    expectedHead:
    ...
    try:
        response_branch = client.get_branch(
            repositoryName=repositoryName,
            branchName=branchName,
        )
        parentCommitId = response_branch["branch"]["commitId"]
        # Create Commit
        put_files, delete_files = _create_commit(
            client,
            input_folder,
            repositoryName,
            subdirectories,
        ...
        if expectedHead != "":
            ...
            if parentCommitId != expectedHead:
                logger.info(
                    "Expected HEAD does not match actual HEAD of repository. Potential merge conflict. Cancel the commit.",
                )
                raise MergeConflict(
                    "Expected HEAD does not match actual HEAD of repository.",
                )
...
def checkout(client, repositoryName, commitId, output_folder):
    ...
    files_in_repo = _traverse_tree_list_files_in_repo(
        [],
        client,
        repositoryName,
        commitId,
...
def _traverse_tree_list_files_in_repo(
    files_in_repo,
    client,
    repositoryName,
    commitId,
    ...
    for item in ls:
        ...
        for file_name in item[-1]:
            ...
            with open(abs_item, "rb") as f:
                put_files.append(
                    {
                        "filePath": abs_item[len(folder) + 1 : :],
                        "fileContent": f.read(),
                    },
                )
    return put_files
