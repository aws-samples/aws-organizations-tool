...
import orgtool.orgs
from orgtool.loginprofile import onetime_passwd_expired
from orgtool.loginprofile import validate_login_profile
from orgtool.loginprofile import validate_user
from orgtool.orgs import validate_accounts_unique_in_org_deployed
from orgtool.reports import account_authorization_report
from orgtool.reports import credentials_report
from orgtool.reports import report_maker
from orgtool.reports import role_report
from orgtool.reports import user_group_report
from orgtool.spec import load_config
from orgtool.spec import validate_spec
from orgtool.utils import get_assume_role_credentials
from orgtool.utils import get_logger
from orgtool.utils import queue_threads
from orgtool.utils import scan_deployed_accounts
from orgtool.utils import string_differ
from orgtool.utils import validate_master_id
...
def expire_users(log, args, deployed, auth_spec, credentials):
    ...
    for name in [u["UserName"] for u in deployed["users"]]:
        ...
        if user:
            ...
            if login_profile and onetime_passwd_expired(
                log,
                user,
                login_profile,
                int(args["--opt-ttl"]),
...
def delete_user(user, iam_client):
    ...
    if (
        "ServiceSpecificCredentials" in response
        and response["ServiceSpecificCredentials"]
    ):
        iam_client.delete_service_specific_credential(
            UserName="string",
            ServiceSpecificCredentialId=response["ServiceSpecificCredentials"][
...
def create_users(credentials, args, log, deployed, auth_spec):
    ...
    for u_spec in auth_spec["users"]:
        ...
        deployed_user = orgtool.utils.lookup(
            deployed["users"],
            "UserName",
            u_spec["Name"],
        )
...
def create_groups(credentials, args, log, deployed, auth_spec):
    ...
    for g_spec in auth_spec["groups"]:
        ...
        deployed_group = orgtool.utils.lookup(
            deployed["groups"],
            "GroupName",
            g_spec["Name"],
        )
        if deployed_group:
            ...
            if orgtool.utils.ensure_absent(g_spec):
                ...
                if list(group.users.all()):
                    log.error(
                        "Can not delete group '%s'. Still contains users"
                        % g_spec["Name"],
                    )
                else:
            ...
            if "Members" in g_spec and g_spec["Members"]:
                if g_spec["Members"] == "ALL":
                    ...
                    for username in g_spec["Members"]:
                        ...
                        if not u_spec:
                            log.error(
                                "User '%s' not in auth_spec['users']. Can not add user to group '%s'"
                                % (username, g_spec["Name"]),
                            )
                        # managed but absent?
                        elif orgtool.utils.ensure_absent(u_spec):
                            log.error(
                                "User '%s' is specified 'absent' in auth_spec['users']. Can not add user to group '%s'"
                                % (username, g_spec["Name"]),
                            )
                        else:
...
def manage_group_policies(credentials, args, log, deployed, auth_spec):
    ...
    iam_client = boto3.client("iam", **credentials)
    iam_resource = boto3.resource("iam", **credentials)
    auth_account = orgtool.utils.lookup(
        deployed["accounts"],
        "Id",
    ...
    for g_spec in auth_spec["groups"]:
        if orgtool.utils.lookup(
            deployed["groups"],
            "GroupName",
            g_spec["Name"],
        ) and not orgtool.utils.ensure_absent(g_spec):
            log.debug(
                "processing group spec for '{}':\n{}".format(g_spec["Name"], g_spec),
            )
            group = iam_resource.Group(g_spec["Name"])
            attached_policies = [
                p.policy_name for p in list(group.attached_policies.all())
            ]
                ...
                for policy_name in g_spec["Policies"]:
                    if policy_name not in attached_policies:
                        ...
                        if policy_arn is None:
                            policy_arn = manage_custom_policy(
                                iam_client,
                                auth_account,
                                policy_name,
                                args,
                        ...
                        log.debug(
                            f"policy Arn for '{policy_name}': {policy_arn}",
                        )
                        log.info(
                            "Attaching policy '%s' to group '%s' in account '%s'"
                            % (policy_name, g_spec["Name"], auth_account),
                        )
                        if args["--exec"]:
                            group.attach_policy(PolicyArn=policy_arn)
                    # update custom policy
                    elif orgtool.utils.lookup(
                        auth_spec["custom_policies"],
                        "PolicyName",
                        policy_name,
                    ):
...
def manage_custom_policy(iam_client, account_name, policy_name, args, log, auth_spec):
    ...
    """
    log.debug(f"account: '{account_name}', policy_name: '{policy_name}'")
    p_spec = orgtool.utils.lookup(
        auth_spec["custom_policies"],
        "PolicyName",
        policy_name,
    )
    ...
    log.debug(
        "account: '%s', custom policies: '%s'"
        % (account_name, [p["Arn"] for p in custom_policies]),
    )
    ...
    # check if custom policy needs updating
        ...
        log.debug(f"account: '{account_name}', policy_doc: {policy_doc}")
        log.debug(f"account: '{account_name}', current_doc: {current_doc}")
        ...
        if current_doc["Statement"] != policy_doc["Statement"]:
            ...
            log.debug(
                "account: {}, update_required: {}".format(
                    account_name,
                    update_required,
                ),
            )
        ...
        if update_required:
            log.info(
                "Updating custom policy '%s' in account '%s':\n%s"
                % (
                    policy_name,
                    account_name,
            ...
            if args["--exec"]:
                log.debug(
                    "check for non-default policy versions for '%s'" % policy_name,
                )
                for v in iam_client.list_policy_versions(PolicyArn=policy["Arn"])[
                    "Versions"
                ]:
                    if not v["IsDefaultVersion"]:
                        log.info(
                            "Deleting non-default policy version '%s' for policy '%s' in account '%s'"
...
def build_resource_list(log, deployed_accounts, d_spec, auth_spec, account_list):
    ...
    for account in account_list:
        ...
        if account_id is not None:
            ...
            log.warn(f"Account {account} not found in deployed accounts")
    return resource
...
def create_group_policy(args, log, group, account, policy_name, policy_doc):
    log.info(
        "Creating assume role policy '{}' for group '{}' in account '{}':\n{}".format(
            policy_name,
            group.name,
            account,
            yamlfmt(policy_doc),
...
def update_group_policy(args, log, group, account, policy_name, policy_doc):
    log.info(
        "Updating policy '{}' for group '{}' in account '{}':\n{}".format(
            policy_name,
            group.name,
            account,
            string_differ(
...
def delete_group_policy(args, log, group, account, policy_name):
    log.info(
        "Deleting assume role group policy '{}' from group '{}' in account '{}'".format(
            policy_name,
            group.name,
            account,
        ),
...
def delete_obsolete_group_policy(
    args,
    log,
    group,
    account,
    ...
    if policy_name not in managed_policies:
        log.info(
            "Deleting obsolete policy '{}' from group '{}' in account '{}'".format(
                policy_name,
                group.name,
                account,
            ),
...
def manage_local_user_in_accounts(
    ...
    log.debug("account: {}, local user: {}".format(account_name, lu_spec["Name"]))

    tags = [{"Key": "contact_email", "Value": lu_spec["ContactEmail"]}]
    ...
    try:
        ...
        user_exists = True
        log.debug(f"account: {account_name}, local user exists: {user.arn}")
    # check for unmanaged user in account
    if user_exists:
        if not user.path.startswith("/" + auth_spec["default_path"]):
            log.error(
                "Can not manage local user '%s' in account '%s'. Unmanaged user with the same name already exists: %s"
    ...
    if account_name not in accounts or orgtool.utils.ensure_absent(lu_spec):
        if user_exists:
            log.info(
                "Deleting local user '{}' from account '{}'".format(
                    user.name,
                    account_name,
                ),
            )
    ...
    if not user_exists:
        log.info(
            "Creating local user '{}' in account '{}'".format(
                lu_spec["Name"],
                account_name,
            ),
        )
        if args["--exec"]:
            ...
            if "Policies" in lu_spec and lu_spec["Policies"]:
                ...
                for policy_name in lu_spec["Policies"]:
                    ...
                    log.info(
                        "Attaching policy '%s' to local user '%s' in account '%s'"
        ...
        for policy_name in attached_policies:
            if policy_name not in lu_spec["Policies"]:
                policy_arn = get_policy_arn(iam_client, policy_name)
                log.info(
                    "Detaching policy '%s' from local user '%s' in account '%s'"
                    % (policy_name, user.name, account_name),
                )
                if args["--exec"] and policy_arn:
...
def manage_local_users(lu_spec, args, log, deployed, auth_spec):
    ...
    for account_name in accounts:
        if not orgtool.utils.lookup(deployed["accounts"], "Name", account_name):
            log.error(
                "Can not manage local user '%s' in account '%s'.  Account '%s' not found in Organization"
...
def get_policies_from_spec(log, auth_spec, d_spec):
    ...
    log.debug(
        "Using PolicySet {} for role {}".format(
            d_spec["PolicySet"],
            d_spec["RoleName"],
        ),
    )
    policy_set = orgtool.utils.lookup(
        auth_spec["policy_sets"],
        "Name",
        d_spec["PolicySet"],
    )
    if policy_set is None:
        log.error(
            "policy set '{}' not found for role '{}'".format(
                d_spec["PolicySet"],
                d_spec["RoleName"],
            ),
        )
...
def update_role_tags(args, log, iam_client, account_name, role, tags):
    ...
    """
    log.debug(
        "role: '{}', account: '{}', role tags: {}; spec tags: {}".format(
            role.name,
            account_name,
            role.tags,
            tags,
    ...
    if tags is not None and role.tags != tags:
        log.info(
            f"Updating tags in role '{role.name}' in account '{account_name}'",
        )
        if args["--exec"]:
...
def create_role(
    ...
    log.info(
        "Creating role '{}' in account '{}'".format(d_spec["RoleName"], account_name),
    )
    if args["--exec"]:
...
def delete_role(args, log, role, account_name):
    log.info(
        f"Deleting role '{role.role_name}' from account '{account_name}'",
    )
    if args["--exec"]:
...
def update_role_path(
    ...
    if role.path != path_spec:
        log.info(
            "Updating path for role '{}' in account '{}'".format(
                role.role_name,
                account_name,
            ),
        )
...
def update_role_policy_document(args, log, role, iam_client, account_name, policy_doc):
    if role.assume_role_policy_document != policy_doc:
        log.info(
            "Updating policy document in role '{}' in account '{}':\n{}".format(
                role.role_name,
                account_name,
                string_differ(
...
def update_role_description(
    ...
    if role.description != role_description:
        log.info(
            "Updating description in role '{}' in account '{}'".format(
                role.role_name,
                account_name,
            ),
        )
...
def update_role_duration(args, log, role, iam_client, account_name, role_duration):
    if role.max_session_duration != role_duration:
        log.info(
            "Updating max session duration in role '{}' in account '{}'".format(
                role.role_name,
                account_name,
            ),
        )
...
def manage_attached_role_policies(
    args,
    log,
    role,
    iam_client,
    ...
    for policy_name in policy_list:
        if policy_name not in attached_policies:
            ...
            log.info(
                "Attaching policy '{}' to role '{}' in account '{}'".format(
                    policy_name,
                    role.name,
                    account_name,
                ),
            ...
            "custom_policies" in auth_spec
            and auth_spec["custom_policies"]
            and orgtool.utils.lookup(
                auth_spec["custom_policies"],
                "PolicyName",
                policy_name,
            )
    ...
    for policy_name in attached_policies:
        if policy_name not in policy_list:
            policy_arn = get_policy_arn(iam_client, policy_name)
            log.info(
                "Detaching policy '{}' from role '{}' in account '{}'".format(
                    policy_name,
                    role.name,
                    account_name,
                ),
...
def get_assume_role_policy_document(d_spec, deployed, auth_spec):
    if "TrustedAccount" in d_spec and d_spec["TrustedAccount"]:
        trusted_account = orgtool.utils.lookup(
            deployed["accounts"],
            "Name",
            d_spec["TrustedAccount"],
            "Id",
        )
    else:
...
def manage_delegations(d_spec, args, log, deployed, auth_spec):
    ...
    if d_spec["TrustingAccount"] == "ALL":
        ...
        if "ExcludeAccounts" in d_spec and d_spec["ExcludeAccounts"]:
            trusting_accounts = [
                a for a in trusting_accounts if a not in d_spec["ExcludeAccounts"]
            ]
    else:
    ...
    for account_name in trusting_accounts:
        if not orgtool.utils.lookup(deployed["accounts"], "Name", account_name):
            log.error(
                "Can not manage delegation role '%s' in account '%s'.  Account '%s' not found in Organization"
    ...
    if "TrustedGroup" in d_spec and "TrustedAccount" in d_spec:
        log.error(
            "can not declare both 'TrustedGroup' or 'TrustedAccount' in delegation spec for role '%s'"
            % d_spec["RoleName"],
        )
        return
    elif "TrustedGroup" not in d_spec and "TrustedAccount" not in d_spec:
        log.error(
            "neither 'TrustedGroup' or 'TrustedAccount' declared in delegation spec for role '%s'"
            % d_spec["RoleName"],
        )
        return
...
def core(args):
    ...
    if args["report"]:
        if args["--account"]:
            deployed["accounts"] = [
                orgtool.utils.lookup(deployed["accounts"], "Name", args["--account"]),
            ]
        if args["--users"]:
            report_maker(
                log,
        ...
        if args["--roles"]:
            report_maker(
                ...
                args["--org-access-role"],
                role_report,
                "IAM Roles and Custom Policies in all Org Accounts:",
                verbose=args["--full"],
            )
        if args["--credentials"]:
            report_maker(
                ...
                args["--org-access-role"],
                credentials_report,
                "IAM Credentials Report in all Org Accounts:",
            )
        if not (args["--users"] or args["--credentials"] or args["--roles"]):
            report_maker(
                ...
                deployed["accounts"],
                args["--org-access-role"],
                account_authorization_report,
                "IAM Account Authorization:",
                verbose=args["--full"],
            )
