...
def list_delegations(log, user, deployed_accounts):
    ...
    for group in user.groups.all():
        ...
        deny_policies = [
            p for p in list(group.policies.all()) if p.name.startswith("DenyAssumeRole")
        ]
...
def format_delegation_table(delegation_arns, deployed_accounts):
    ...
    for assume_role_arn in delegation_arns:
        ...
        if alias is None:
            alias = ""
        spacer = (24 - len(alias)) * " "
        delegation_string += template.format(
            account_id,
            alias,
            spacer,
...
def set_access_key_status(log, user, enable=True):
    ...
    for key in user.access_keys.all():
        if enable and key.status == "Inactive":
            log.info(
                "enabling access key {} for user {}".format(
                    key.access_key_id,
                    user.name,
                ),
            )
            ...
            log.info(
                "disabling access key {} for user {}".format(
                    key.access_key_id,
                    user.name,
                ),
            )
...
def prep_email(log, aliases, deployed_accounts, user, passwd):
    ...
    template = os.path.abspath(
        pkg_resources.resource_filename(__name__, EMAIL_TEMPLATE),
    )
    mapping = dict(
        user_name=user.name,
        onetimepw=passwd,
