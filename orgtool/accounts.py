...
from orgtool.spec import yamlfmt
...
from orgtool.utils import get_s3_bucket_name
from orgtool.utils import lookup
from orgtool.utils import merge_aliases
from orgtool.utils import queue_threads
from orgtool.utils import scan_created_accounts
            ...
            if args["--exec"]:
                ...
                while counter < maxtries:
                    ...
                    if creation["State"] == "IN_PROGRESS":
                        time.sleep(5)
                        log.info(
                            "Account creation in progress for '%s'" % a_spec["Name"],
                        )
                        ...
                        log.error(
                            "Account creation failed: %s" % creation["FailureReason"],
                        )
                        break
                    counter += 1
...
def set_account_tags(account, log, args, account_spec, org_client):
    ...
    log.debug(
        'tag_spec for account "{}":\n{}'.format(
            account["Name"],
            yamlfmt(tag_spec),
        ),
    )
    log.debug(
        'account_tags for account "{}":\n{}'.format(
            account["Name"],
...
def set_account_alias(account, log, args, account_spec, role):
    ...
    log.debug("account_name: {}; aliases: {}".format(account["Name"], aliases))
    if not aliases and proposed_alias:
        log.info(
            "setting account alias to '%s' for account '%s'"
...
def invite_account(log, args, org_client, deployed_accounts):
    ...
    if account_invite:
        ...
        if invite_state in ["REQUESTED", "OPEN"]:
            log.error(
                "Account %s has already been invited to Org and status is %s"
    ...
    if args["--exec"]:
        ...
        handshake = org_client.invite_account_to_organization(Target=target)[
            "Handshake"
        ]
...
def display_provisioned_accounts(log, deployed_accounts, status):
    ...
    sorted_account_names = sorted(
        [a["Name"] for a in deployed_accounts if a["Status"] == status],
    )
    if sorted_account_names:
        ...
        for name in sorted_account_names:
            ...
            log.info(
                fmt_str.format(name, account["Alias"], account["Id"], account["Email"]),
            )
