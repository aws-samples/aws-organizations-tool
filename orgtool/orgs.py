#!/usr/bin/env python
"""Manage resources in an AWS Organization and Organization Unit tree.
...
from orgtool.utils import ensure_absent
from orgtool.utils import flatten_OUs
from orgtool.utils import get_account_aliases
from orgtool.utils import get_assume_role_credentials
from orgtool.utils import get_logger
from orgtool.utils import get_root_id
from orgtool.utils import lookup
from orgtool.utils import scan_deployed_accounts
from orgtool.utils import scan_deployed_tags_for_resource
from orgtool.utils import search_spec
...
def validate_accounts_unique_in_org_deployed(log, deployed_accounts):
    ...
    for account in deployed_accounts:
        if account["Name"] not in duplicate_values:
            ...
            if count > 1:
                ...
                log.error(
                    "Invalid deployed org: Account name should be unique. Duplicate account name '%s' found for Ids %s."
                    % (account["Name"], str(account_ids)),
                )
    ...
    #             if count > 1:
    #                 duplicate = True
    #                 log.error("Invalid deployed org: Account name should be unique. Duplicate account name '%s' found" % (account_name))

    if duplicate:
        log.critical("Invalid deployed org")
        log.critical(
            "Duplicate account name found (%s). orgtool doesn't support multi account name in the same org."
            % (str(duplicate_values)),
        )
        log.info(
            "You could rename the account by following the AWS documentation at https://aws.amazon.com/premiumsupport/knowledge-center/change-organizations-name/",
        )
        sys.exit(1)
...
def enable_policy_type_in_root(org_client, root_id):
    ...
    if not p_type or (
        p_type[0]["Type"] == "SERVICE_CONTROL_POLICY"
...
def get_parent_id(org_client, account_id):
    ...
    try:
        ...
        raise RuntimeError(
            "API Error: account '%s' has more than one parent: %s"
            % (account_id, parents),
        )
...
def build_deployed_ou_table(
    ...
    # recursive sub function to build the 'deployed_ou' table
    ...
    while "NextToken" in response and response["NextToken"]:
        ...
        accounts += response["Accounts"]
    log.debug(f"parent_name: {parent_name}; ou: {yamlfmt(child_ou)}")
    log.debug(f"parent_name: {parent_name}; accounts: {yamlfmt(accounts)}")
    ...
    if not deployed_ou:
        deployed_ou.append(
            dict(
                ...
                Child_OU_Path=[
                    (parent_path + "/" + ou["Name"]) for ou in child_ou if "Name" in ou
                ],
        ...
        for ou in deployed_ou:
            if ou["Path"] == parent_path:
                ...
                ou["Child_OU_Path"] = [
                    (parent_path + "/" + d["Name"]) for d in child_ou
                ]
...
def reverse_ou(org_client, log, deployed, ou_path, default_sc_policy):
    ...
    if "Child_OU_Path" in deployed_ou and len(deployed_ou["Child_OU_Path"]) > 0:
        ou["Child_OU"] = [
            reverse_ou(org_client, log, deployed, child_OU_Path, default_sc_policy)[0]
            for child_OU_Path in deployed_ou["Child_OU_Path"]
        ]
        ...
        policies.append(
            dict(
                ...
                Statement=json.loads(
                    org_client.describe_policy(PolicyId=policy["Id"])["Policy"][
                        "Content"
                    ],
                )["Statement"],
            ),
...
def reverse_accounts(org_client, log, deployed, org_access_role):
    ...
    for account in deployed["accounts"]:
        if account["Status"] == "ACTIVE":
            ...
            log.info(
                "Account %s (%s) is %s, then not added to the configuration"
        ...
        for account in ou_spec["Accounts"]:
            ...
            if not account_id:
                ...
                if dest_parent_id != source_parent_id:
                    ...
                    log.info(
                        f"Moving account '{account}' to OU '{ou_spec_path}'",
                    )
                    if args["--exec"]:
...
def place_unmanged_accounts(org_client, args, log, deployed, account_list, dest_parent):
    ...
    """
    log.warn(
        "move_unmanaged_account: True - New config to control if unmanaged account move to default OU",
    )
    for account in account_list:
        ...
        if dest_parent_id and dest_parent_id != source_parent_id:
            log.info(
                "Moving unmanged account '%s' to default OU '%s'"
                % (account, dest_parent),
            )
            if args["--exec"]:
                org_client.move_account(
    ...
    for p_spec in org_spec["sc_policies"]:
        ...
        if ensure_absent(p_spec) and withdelete:
            if policy:
                ...
                if org_client.list_targets_for_policy(PolicyId=policy["Id"])["Targets"]:
                    log.error(
                        "Cannot delete policy '%s'. Still attached to OU" % policy_name,
                    )
                elif args["--exec"]:
        ...
        if not policy:
            ...
            if args["--exec"]:
                ...
                log.info(
                    f"Creating policy '{policy_name}' response: {response}",
                )
        # check for policy updates
        else:
            deployed_policy_doc = json.dumps(
                json.loads(
                    org_client.describe_policy(PolicyId=policy["Id"])["Policy"][
                        "Content"
                    ],
            ...
            if (
                p_spec["Description"] != policy["Description"]
                ...
                if args["--exec"]:
                    ...
                    log.info(
                        f"Update policy '{policy_name}' response: {response}",
                    )


def manage_policy_attachments(
    org_client,
    args,
    log,
    deployed,
    ...
    policies_to_detach = [
        p
        for p in attached_policy_list
        if p not in spec_policy_list and p != org_spec["default_sc_policy"]
    ]
    # attach policies
    for policy_name in policies_to_attach:
        if not lookup(deployed["policies"], "Name", policy_name):
            ...
            if args["--exec"]:
                log.error(
                    "Error in config '{}', the policy '{}' can't be attached to the OU '{}' because the policy is not defined".format(
                        args["--config"],
                        policy_name,
                        ou_spec["Path"],
                    ),
                ...
                log.warn(
                    "Error in config '{}', the policy '{}' couldn't be attached to the OU '{}' because the policy is not defined".format(
                        args["--config"],
                        policy_name,
                        ou_spec["Path"],
                    ),
        ...
        if not ensure_absent(ou_spec):
            log.info(
                f"Attaching policy '{policy_name}' to OU '{ou_spec_path}'",
            )
    ...
    for policy_name in policies_to_detach:
        log.info(f"Detaching policy '{policy_name}' from OU '{ou_spec_path}'")
...
def manage_ou(
    ...
    for ou_spec in ou_spec_list:
            ...
            if ensure_absent(ou_spec):
                ...
                for key in ["Accounts", "SC_Policies", "Child_OU"]:
                    ...
                    if key in ou and ou[key]:
                        if key == "SC_Policies":
                            log.error(
                                "Delete OU '%s'. Deployed '%s' will be unattach."
                                % (ou_spec["Path"], key),
                            )
                        ...
                        if deployed["ou"][i]["Path"] == parent_path:
                            ...
                            for ii, iitem in enumerate(deployed["ou"][i]["Child_OU"]):
                                if deployed["ou"][i]["Child_OU"][ii] == ou_spec["Name"]:
                                    ...
                                    break
                            for iii, iiitem in enumerate(
                                deployed["ou"][i]["Child_OU_Path"],
                            ):
                                if (
                                    deployed["ou"][i]["Child_OU_Path"][iii]
                                    == ou_spec["Path"]
                                ):
        ...
        # create new OU
            ...
            log.info(
                "Creating new OU '%s' under parent '%s'"
                % (ou_spec["Path"], parent_name),
            )
            ...
            if args["--exec"]:
                ...
                if "Child_OU" in ou_spec and ou_spec["Child_OU"]:
                    Child_OUs = [
                        ou["Name"] for ou in ou_spec["Child_OU"] if "Name" in ou
                    ]
                    Child_OUs_Path = [
                        (parent_path + "/" + ou["Name"])
...
def set_ou_tags(ou, log, args, ou_spec, org_client):
    ...
    if sorted_tags(ou_tags) != sorted_tags(tag_spec):
        log.info(
            "New feature for tagging OUs - Laurent Delhomme <delhom@amazon.com> AWS June 2020",
        )
        log.info(
            'Updating tags for OU "{}":\n{}'.format(
                ou_spec["Path"],
        ...
        log.debug(
            'Deployed tags == tag-spec. So doing nothing for OU "{}".'.format(
                ou_spec["Path"],
            ),
        )
...
def core(args):
    ...
    if args["report"]:
        log.info(
            "To get files, use the command orgtoolconfigure reverse-setup --template-dir <path> --output-dir <path> [--force] --master-account-id <id> --org-access-role <role> [--exec] [-q] [-d|-dd]",
        )
        log.info(
            "The package provide a template for this purpose located into the folder 'spec_init_data.reverse'",
        )
        log.info("The output files will contain the extract of OUs, SCPs and Accounts")
        log.info(
            "Other resources (delegation, users, local_users ...) will not be updated by the exploration of the organization",
        )
    ...
    if args["organization"]:
        ...
        for key in list(managed.keys()):
            if key == "accounts":
                unmanaged = [
                    a["Name"] for a in deployed[key] if a["Name"] not in managed[key]
                ]
                if unmanaged:
                    log.warn(
                        "Unmanaged {} in Organization: {}".format(
                            key,
                            ", ".join(unmanaged),
                        ),
                    )
                    ...
                    if org_spec["move_unmanaged_account"]:
                        ...
                        log.info(
                            "Updated code, move_unmanaged_account set to False therefore unmanged account not moved to default OU - Laurent Delhomme <delhom@amazon.com> AWS June 2020",
                        )
            ...
            if key == "ou":
                ...
                unmanaged_path = [
                    a["Path"] for a in deployed[key] if a["Path"] not in managed[key]
                ]
                if unmanaged:
                    log.warn(
                        "Unmanaged %s in Organization: %s"
                        % (key, ", ".join(unmanaged_path)),
                    )
                    ...
                    while len(unmanaged) != 0:
                        ...
                        if protection > protection_max:
                            log.critical(
                                "Throw exception as a protection of the program. Too many loops to remove unmanaged OUs.",
                            )
                            sys.exit(1)
                            # if args['--exec']:
                            #
                        ...
                        for i, item in enumerate(unmanaged):
                            ...
                            if (
                                "Child_OU" in unmanaged[i]
                                and len(unmanaged[i]["Child_OU"]) != 0
                            ):
                                log.critical(
                                    "Not able to delete OU %s because contains OU %s"
                                    % (
                                        unmanaged[i]["Path"],
                                        ", ".join(unmanaged[i]["Child_OU"]),
                                    ),
                                )
                                ...
                                if (
                                    "Accounts" in unmanaged[i]
                                    and len(unmanaged[i]["Accounts"]) != 0
                                ):
                                    log.critical(
                                        "Not able to delete OU %s because contains accounts %s"
                                        % (
                                            unmanaged[i]["Path"],
                                            ", ".join(unmanaged[i]["Accounts"]),
                                        ),
                                    )
                                    ...
                                    if args["--exec"]:
                                        ...
                                        log.info(
                                            "dryrun - then continu - not change will be applied",
                                        )

                                # then delete the OU
                                if args["--exec"]:
                                    org_client.delete_organizational_unit(
                                        OrganizationalUnitId=unmanaged[i]["Id"],
                                    )
                                # update deployed structure
                                ...
                                for ii, iitem in enumerate(unmanaged):
                                    if unmanaged[ii]["Path"] == ou_parent_path:
                                        # remove the child_OU ref in the parent
                                        for iii, iiitem in enumerate(
                                            unmanaged[ii]["Child_OU"],
                                        ):
                                            if (
                                                unmanaged[ii]["Child_OU"][iii]
                                                == ou_name
                                            ):
                                                unmanaged[ii]["Child_OU"].pop(iii)
                                                break
                                        for iiii, iiiitem in enumerate(
                                            unmanaged[ii]["Child_OU_Path"],
                                        ):
                                            if (
                                                unmanaged[ii]["Child_OU_Path"][iiii]
                                                == ou_path
                                            ):
