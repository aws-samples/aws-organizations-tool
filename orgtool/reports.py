...
from orgtool.loginprofile import user_report
...
from orgtool.utils import get_iam_objects
...
def overbar(string):
    """
    Returns string proceeded by an overbar of the same length:
    >>> print(overbar('blee'))
    ____
    blee
    """
...
def credentials_report(credentials):
    ...
    for row in reader:
        ...
        for key in reader.fieldnames:
            ...
            user["Arn"] = row["arn"]
            if key not in ["user", "arn"] and row[key] not in [
                "N/A",
                "not_supported",
                "no_information",
                "false",
        ...
        if verbose:
            role_info.append(
                dict(
                    ...
                    AttachedPolicies=[
                        p.policy_name for p in list(role.attached_policies.all())
                    ],
                ),
            )
        else:
...
def display_provisioned_groups(log, args, deployed, credentials):
    ...
    def display_group(group_name, report, iam_resource):
        ...
        assume_role_resources = [
            p.policy_document["Statement"][0]["Resource"]
            for p in list(group.policies.all())
            if p.policy_document["Statement"][0]["Action"] == "sts:AssumeRole"
        ]
        overbar = "_" * (8 + len(group_name))
        messages.append("\n%s" % overbar)
        messages.append("{}\t{}".format("Name:", group_name))
        messages.append("{}\t{}".format("Arn:", group.arn))
        if members:
        ...
        if isinstance(credentials, RuntimeError):
            ...
            for r in roles:
                ...
                if not args["--full"]:
                    ...
                    principal = role.assume_role_policy_document["Statement"][0][
                        "Principal"
                    ]
                    if "AWS" in principal:
