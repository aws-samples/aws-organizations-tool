#!/usr/bin/env python
"""Configure resources in an AWS Organization description files.
...
Usage:
  ...
  orgtoolconfigure delegation (delete|create --trusted-account <name> --account-name <name>... --description <description> [--require-mfa] [--ensure-present | --ensure-absent] --policies <policy-name>...) --config <path> --role-name <name> [--exec] [-q] [-d|-dd]
...
Modes of operation:
  ...
  distributed-config create          create a distributed config included in the parent config
  ...
  validate                           validate the configuration and recursively all distributed config included
...
Options:
  ...
  --email <email>
  --alias <alias>
  --tag <key>=<value>         list of account tags to be added in a structure of key=value
...
import orgtool
from orgtool.orgs import reverse_accounts
from orgtool.orgs import reverse_ou
from orgtool.orgs import reverse_policies
from orgtool.orgs import scan_deployed_accounts
from orgtool.orgs import scan_deployed_ou
from orgtool.orgs import scan_deployed_policies
from orgtool.spec import load_config
from orgtool.spec import validate_spec
from orgtool.spec import validate_spec_dict
from orgtool.utils import dump_to_spec_config
...
def reverse_setup(args, log):
    ...
    if "--template-dir" in args and args["--template-dir"]:
        ...
        if not os.path.isdir(template_dir):
            log.error(f"template_dir not found: {template_dir}")
    ...
    if "--output-dir" in args and args["--output-dir"]:
        ...
        if os.path.isdir(output_dir):
            if "--force" in args and args["--force"]:
                log.info(
                    "With '--force', then delete existing output directory '{}'".format(
                        output_dir,
                    ),
                )
                if args["--exec"]:
                ...
                log.error(
                    "Output directory '{}' exists and could be not empty. Refusing to overwrite. Use '--force' to force overwrite".format(
                        output_dir,
                    ),
                )
                raise Exception(
    ...
    reverse_config = dict(
        organizational_units=reverse_ou(
            org_client,
            log,
            deployed,
            "/root",
    ...
    for key in config_keys:
        ...
        if errors:
            log.critical(
                "The organization configuration is not compliant with orgtool limitation for {}. Run in debug mode for details".format(
                    key,
                ),
            )
            sys.exit(1)
    ...
    if args["--exec"]:
        ...
        f = open(config_file_common)
        fc = f.read()
        fc = fc.replace("000000000000", args["--master-account-id"])
        f.close()
        f = open(config_file_common, "w")
        f.write(fc)
        f.close()
    ...
    if args["--exec"]:
        log.info(
            "orgtool reverse-setup executed with success. Files delivered in {}".format(
                output_dir,
            ),
        )
...
def distributed_config_create(args, log, org_spec):
    ...
    if (
        "organizational_units" in child_org_spec
        and child_org_spec["organizational_units"]
    ):
...
def distributed_config_delete(args, log, org_spec):
    ...
    if OUs[child_ou_path] and OUs[args["--ou-path"]]:
        ...
        log.error(f"'{child_ou_path}' not found in org_spec OUs")
        raise Exception(f"'{child_ou_path}' not found in org_spec OUs")
        # sys.exit(-1)
...
def account_create(args, log, org_spec):
    ...
    if "--tag" in args and args["--tag"]:
        ...
        for tag in args["--tag"]:
            tab = str(tag).split("=")
            if len(tab) != 2:
                log.critical(f"'{tab}' not formatted as key=value")
                sys.exit(1)
            else:
                key = tab[0]
                value = tab[1]
...
def account_update(args, log, org_spec):
    ...
    if "accounts" in org_spec and org_spec["accounts"]:
        ...
        account = [
            a for a in org_spec["accounts"] if a["Name"] == args["--account-name"][0]
        ]
        if len(account) == 1:
            if len(args["--alias"]) == 0:
            ...
            log.error(
                "'{}' not found in org_spec accounts".format(args["--account-name"][0]),
            )
            raise Exception(
                "'{}' not found in org_spec accounts".format(args["--account-name"][0]),
            )
        ...
        for i, index in enumerate(from_org_spec["accounts"]):
            if from_org_spec["accounts"][i]["Name"] == args["--account-name"][0]:
                ...
                if args["--ou-path"] in OUs and OUs[args["--ou-path"]]:
                        ...
                        if "Accounts" in from_ou and from_ou["Accounts"]:
                            for ii, iindex in enumerate(from_ou["Accounts"]):
                                if from_ou["Accounts"][ii] == args["--account-name"][0]:
                                    ...
                                    # add account to dest
                                    if (
                                        "Accounts" in OUs[args["--ou-path"]]
                                        and OUs[args["--ou-path"]]["Accounts"]
                                    ):
                                        OUs[args["--ou-path"]]["Accounts"] += args[
                                            "--account-name"
                                        ]
                                    else:
                                        OUs[args["--ou-path"]]["Accounts"] = args[
                    ...
                    log.error(
                        "'{}' not found in org_spec OUs".format(args["--account-name"]),
                    )
                    raise Exception(
                        "'{}' not found in org_spec OUs".format(args["--account-name"]),
                    )
                    ...
                    raise Exception(
                        "'{}' not found in org_spec".format(args["--ou-path"]),
                    )
                    # sys.exit(-1)

    if not found:
        log.error(
            "Account '{}' not found in the organisation's accounts".format(
                args["--account-name"][0],
            ),
        )
        raise Exception(
...
def delegation_create(args, log, org_spec):
    # orgtoolconfigure delegation create --trusted-account <name> --account-name <name>... --description <description> [--require-mfa] [--ensure-present --ensure-absent] --policies <policy-name>... --config <path> --role-name <name> [--exec] [-q] [-d|-dd]
    ...
    if "delegations" in org_spec and org_spec["delegations"]:
        delegations = [
            d for d in org_spec["delegations"] if d["RoleName"] == args["--role-name"]
        ]
...
def delegation_delete(args, log, org_spec):
    ...
    if not found:
        log.error(
            "Delegation '{}' not found in the organisation's delegations '{}'".format(
                args["--role-name"],
                args["--config"],
            ),
        )
                ...
                for i, index in enumerate(args["--account-name"]):
                    if (
                        len(
                            [
                                a
                                for a in delegation["TrustingAccount"]
                                if a == args["--account-name"][i]
                            ],
                        )
                        == 0
                    ):
                        ...
                        log.debug(
                            "Account '{}' is already in the trusting account list of '{}'".format(
                                args["--account-name"][i],
                                args["--role-name"],
                            ),
                        )
    ...
    if "delegations" in org_spec and org_spec["delegations"]:
        ...
        if len(delegations) == 1:
            ...
            if "TrustingAccount" in delegation and delegation["TrustingAccount"]:
                ...
                log.debug(
                    "Trusting account list of '{}' is empty".format(
                        args["--role-name"],
                    ),
                )
...
def organization_unit_create(args, log, org_spec):
    ...
    if args["--ou-path"] in OUs and OUs[args["--ou-path"]]:
        log.error(
            "OU '{}' already found in the organisation '{}'".format(
                args["--ou-path"],
                args["--config"],
            ),
        )
        ...
        if parent_path in OUs and OUs[parent_path]:
            ...
            log.error(
                "OU '{}' not found in the organisation '{}'".format(
                    parent_path,
                    args["--config"],
                ),
            )
            ...
            for i, index in enumerate(args["--scp-name"]):
                if (
                    len(
                        [
                            scp
                            for scp in ou["SC_Policies"]
                            if scp == args["--scp-name"][i]
                        ],
                    )
                    == 0
                ):
                    ...
                    log.debug(
                        "SCP '{}' is already in the SC_Policies list of '{}'".format(
                            args["--scp-name"][i],
                            args["--ou-path"],
                        ),
                    )
            ...
            for tag in args["--tag"]:
                ...
                if len(tab) != 2:
                    ...
                    if "Tags" in ou and ou["Tags"]:
                        ...
                        log.debug(
                            "Trying to remove Tag with key "
                            + key
                            + " for OU "
                            + ou["Name"]
                            + " but OU has no Tags set. So doing nothing.",
                        )
...
def core(args, log=None):
    ...
    log.debug(args)
    log.info(
        "New feature for IaC, CLI to manipulate the configuration - Laurent Delhomme <delhom@amazon.com> AWS June 2020",
    )
